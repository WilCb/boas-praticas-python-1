...
def listar_produtos() -> List[Produto]:
    ...
    Returns:
        List[Produtos]: Uma lista de objetos de produtos cadastrados.
    """
    return produtos
...
def adicionar_historico_compras(usuario_id: int, compras: HistoricoCompras) -> Dict[str, str]:
    ...
    Returns:
        Dict[str, str]: Mensagem indicando que o histórico de compras foi atualizado.
    """
...
def recomendar_produtos(usuario_id: int, preferencias: Preferencias) -> List[Produto]:
    ...
    Args:
        usuario_id (int): O ID do usuário para qual as recomendações serão feitas.
    ...
    produto_recomendados_filtrados = []
    for produto in produtos_recomendados_categorias:
        for tag in produto.tags:
            if tag in preferencias.tags:
                produto_recomendados_filtrados.append(produto)
                break

    return produto_recomendados_filtrados
